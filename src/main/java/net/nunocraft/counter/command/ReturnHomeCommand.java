...
public class ReturnHomeCommand {
    ...
    private int returnHome(CommandSourceStack source) throws CommandSyntaxException {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray(Counter.MOD_ID + "homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray(Counter.MOD_ID + "homepos");
